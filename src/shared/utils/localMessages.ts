...
export interface Message {
    ...
    agentStatus?: 'running' | 'completed' | 'failed' | 'cancelled';
...
// Persist the turn ID before a request so a refresh or retry uses the same identity.
export const ensureAgentTurnId = (message: Message): string => {
    if (message.agentTurnId) return message.agentTurnId
    const turnId = uuidv4()
    const conversations = getLoclMessages()
    const conversation = conversations.find((item) => item.covId === getSelectId())
    const users = conversation?.data.filter((item) => item.role === 'user') || []
    const lastUser = users[users.length - 1]
    if (lastUser && lastUser.content === message.content && lastUser.timestamp === message.timestamp) {
        lastUser.agentTurnId = turnId
